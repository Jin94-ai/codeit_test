...
# 설정
# ==============================
LABEL_ZIP_ROOT = BASE_DIR / (
    "166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터"
    "/01.데이터/1.Training/라벨링데이터/경구약제조합 5000종"
)
IMAGE_ZIP_ROOT = BASE_DIR / (
    "166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터"
    "/01.데이터/1.Training/원천데이터/경구약제조합 5000종"
)
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        for member in zf.namelist():
            ...
            try:
                ...
                dl_idx = int(img_info["dl_idx"])
                dl_name = img_info["dl_name"]

                # ==============================
                # categories 수정
                # ==============================
                data["categories"][0]["id"] = dl_idx
                data["categories"][0]["name"] = dl_name
...
print(f"✓ JSON 스킵: {json_skip_count}개 (빈/손상)")
